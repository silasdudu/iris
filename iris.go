...
func New(setters ...OptionSetter) *Framework {
...

	{
...

		{
			...
			// serializer content-types(json,jsonp,xml,markdown,text,data) the defaults are setted:
			...
			s.Adapt(RenderPolicy(func(out io.Writer, name string, bind interface{}, options ...map[string]interface{}) (error, bool) {
				...
				if err != nil {
					return nil, false //don't alt the error if failed
				}
				if len(b) > 0 {
					_, err = out.Write(b)
					return err, true
		...
		s.Adapt(EventPolicy{Build: func(*Framework) {
			...
			if s.Router.handler == nil {
				...
				if s.policies.RouterBuilderPolicy == nil {
					...
					os.Exit(0) // don't trace anything else.
				}
				// buid the router using its policy
